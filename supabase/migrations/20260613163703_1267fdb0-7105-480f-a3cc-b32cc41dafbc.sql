...
-- Restrict cms-media bucket uploads to admins only
...
WITH CHECK (bucket_id = 'cms-media' AND public.is_admin(auth.uid()));

CREATE POLICY "Admins can update cms media"
ON storage.objects FOR UPDATE TO authenticated
USING (bucket_id = 'cms-media' AND public.is_admin(auth.uid()))
WITH CHECK (bucket_id = 'cms-media' AND public.is_admin(auth.uid()));
