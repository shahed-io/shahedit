  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "linear-gradient(180deg, #0a0a1a, #141432)" }}>
      ...
      <main className="flex-1 container mx-auto px-4 py-10 max-w-5xl">
        ...
        {q && (
          <Link to={`/ai-search?q=${encodeURIComponent(q)}`}
            className="group flex items-center gap-3 rounded-2xl px-4 py-3 mb-5 transition hover:scale-[1.005]"
            style={{ background: "linear-gradient(135deg, rgba(168,85,247,0.15), rgba(236,72,153,0.10))", border: "1px solid rgba(168,85,247,0.35)" }}>
            <span className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
              style={{ background: "linear-gradient(135deg, #a855f7, #ec4899)", boxShadow: "0 4px 12px rgba(236,72,153,0.40)" }}>
              <Sparkles size={16} className="text-white" />
            </span>
            <span className="flex-1 min-w-0">
              <span className="block text-sm font-semibold text-white">AI দিয়ে "{q}" এর উত্তর পান</span>
              <span className="block text-xs text-slate-400 mt-0.5">Bangla/English-এ প্রশ্ন করুন — AI অ্যাসিস্ট্যান্ট সরাসরি উত্তর দেবে</span>
            </span>
            <ArrowUpRight size={16} className="text-purple-300 shrink-0" />
          </Link>
        )}
